...
set -e
if [ "$#" -ne 4 ]; then
  echo "Usage: Call this script with 4 parameters e.g. init-tf.sh bucket_name file_name(in format git_repo/env_name/user/terraform.tfstate) dynamodb_tab_name region"
  exit 1
fi
